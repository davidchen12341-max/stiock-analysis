...
- `/trade watchlist add <tickers>` — add one or more tickers (comma-separated: `AAPL, NVDA, TSLA`)
- `/trade watchlist remove <tickers>` — remove one or more tickers (comma-separated: `AAPL, NVDA`)
...
**Multi-ticker input:** The argument may be a single ticker or multiple tickers separated by `", "` (comma + space). Parse the input by splitting on `", "` to get the list. Examples:
- `/trade watchlist add MU` → adds MU
- `/trade watchlist add AAPL, NVDA, TSLA` → adds all three

When adding tickers:
1. Parse the input: split on `", "` to extract individual ticker symbols
2. Validate each ticker exists via WebSearch
3. Check if already on watchlist (skip duplicates with message)
4. Run full Quick Score assessment for each new ticker — **fire all WebSearches in parallel** across tickers for speed
5. Identify key support/resistance levels for alert tracking
6. Add all new tickers to watchlist file in score-ranked order
7. User can optionally provide:
...
**Multi-ticker input:** Same parsing as add — split on `", "` to handle multiple tickers at once.

When removing tickers:
1. Parse the input: split on `", "` to extract individual ticker symbols
2. Confirm each ticker is on the watchlist (warn if not found, continue with others)
3. Remove all matched tickers from file
4. Note each removal with final score: "[TICKER] removed from watchlist (last score: XX)"
